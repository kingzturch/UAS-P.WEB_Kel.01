...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE pemantauan SET pegawai_id=?, kekeruhan=?, dis_o2=?, pH=?, suhu=?, tgl_pantau=?, lokasi_id=?, status=? WHERE id=?";

    if ($stmt = mysqli_prepare($koneksi, $sql)) {
        // Bind variabel ke pernyataan persiapan sebagai parameter
        mysqli_stmt_bind_param($stmt, "iddddsisi", $param_pegawai_id, $param_kekeruhan, $param_dis_o2, $param_pH, $param_suhu, $param_tgl_pantau, $param_lokasi_id, $param_status, $param_id);
        ...
        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Data berhasil diubah'); window.location.href = 'hasil_pemantauan.php';</script>";
